...
function App() {
  ...
  const overdueTasks = tasks?.filter(t => !t.done && new Date(t.due_date) < new Date()).length || 0;

  const handleCreateTask = async (data: { title: string; content: string; due_date: string }) => {
    await createTask(data);
    setShowForm(false);
    refetch();
  };

  const handleUpdateTask = async (data: { title: string; content: string; due_date: string }) => {
    if (editingTask) {
